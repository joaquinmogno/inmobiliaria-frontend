...
import PlanLifecycleModal, { type PlanLifecycleAction } from "./PlanLifecycleModal";
...
export interface ContractDetailsModalProps {
    ...
    onRescind?: (contract: Contract) => void;
...
export default function ContractDetailsModal({
    ...
    const [planLifecycleAction, setPlanLifecycleAction] = useState<{ plan: PlanCuotas; action: PlanLifecycleAction } | null>(null);
    ...
    const hasRenewedContract = (contract.historialRenovaciones || []).some(entry => entry.contratoAnteriorId === contract.id);

    const principalDocuments = (contract.adjuntos || []).filter(document => document.tipo === 'CONTRATO_PRINCIPAL');
    const currentPrincipalDocument = principalDocuments.find(document => document.esVigente)
        || principalDocuments.find(document => document.rutaArchivo === contract.rutaArchivoContrato);
    const previousPrincipalDocuments = principalDocuments.filter(document => document.id !== currentPrincipalDocument?.id);
    const complementaryDocuments = (contract.adjuntos || []).filter(document => document.tipo !== 'CONTRATO_PRINCIPAL');

    const loadAuditPage = async (page: number) => {
        setIsLoadingAudit(true);
        try {
            const details = await contractsService.getById(contract.id, page);
            setCurrentContract(current => current ? {
                ...current,
                auditLogs: details.auditLogs,
                auditMeta: details.auditMeta
            } : details);
        } catch (error) {
    ...
    const getStatusBadge = (estado: string) => {
        switch (estado) {
            case 'BORRADOR': return <span className="bg-gray-50 text-content-muted px-2 py-0.5 rounded text-xs font-black border border-gray-200 uppercase tracking-widest">Borrador</span>;
            ...
            case 'CONFIRMADA': return <span className="bg-indigo-50 text-indigo-700 px-2 py-0.5 rounded text-xs font-black border border-indigo-200 uppercase tracking-widest">Confirmada</span>;
            case 'ANULADA': return <span className="bg-red-50 text-red-700 px-2 py-0.5 rounded text-xs font-black border border-red-200 uppercase tracking-widest">Anulada</span>;
            default: return <span className="text-gray-600 text-xs font-bold uppercase tracking-widest">{getStatusLabel(estado)}</span>;
        }
    };
    ...
    const getContractStatusBadge = (estado: Contract['estado']) => {
        ...
        return <span className={`text-xs uppercase px-2 py-0.5 rounded border ${styles[estado]}`}>{getStatusLabel(estado)}</span>;
    };

    const handlePlanLifecycle = async (data: { motivo: string; montoTotal?: number; cantidadCuotas?: number; fechaPrimeraCuota?: string }) => {
        if (!canEditLiquidations || !planLifecycleAction) return;
        const { plan, action } = planLifecycleAction;
        try {
            if (action === "CANCELAR") await planesCuotasService.cancelar(plan.id, data.motivo);
            if (action === "CONDONAR") await planesCuotasService.condonar(plan.id, data.motivo);
            if (action === "REPROGRAMAR") {
                await planesCuotasService.reprogramar(plan.id, {
                    motivo: data.motivo,
            ...
            toast.success(action === "REPROGRAMAR" ? "Plan reprogramado y vinculado al nuevo acuerdo" : action === "CONDONAR" ? "Cuotas pendientes condonadas" : "Plan cancelado");
            setPlanLifecycleAction(null);
            await loadPlanesCuotas();
        } catch (error: any) {
            toast.error(error.response?.data?.message || "No se pudo actualizar el plan");
        }
    };

    const getPlanStatusBadge = (estado: PlanCuotas["estado"]) => {
        const styles = {
            ...
            REPROGRAMADO: "bg-violet-50 text-violet-700 border-violet-200",
            CONDONADO: "bg-teal-50 text-teal-700 border-teal-200"
        };
        const labels = { VIGENTE: "Vigente", CUMPLIDO: "Cumplido", CANCELADO: "Cancelado", REPROGRAMADO: "Reprogramado", CONDONADO: "Condonado" };
        return <span className={`rounded border px-1.5 py-0.5 text-[10px] font-black uppercase tracking-wider ${styles[estado]}`}>{labels[estado]}</span>;
    };

    const getInstallmentStatusBadge = (estado: NonNullable<PlanCuotas["cuotas"]>[number]["estado"]) => {
        const styles = {
            PENDIENTE: "bg-gray-100 text-gray-600",
            PAGADA: "bg-green-50 text-green-700",
            CANCELADA: "bg-orange-50 text-orange-700",
            REPROGRAMADA: "bg-violet-50 text-violet-700",
            CONDONADA: "bg-teal-50 text-teal-700"
        };
        return <span className={`rounded px-1.5 py-0.5 text-xs font-bold ${styles[estado]}`}>{estado}</span>;
    };
    ...
    return (
                            ...
                            <Dialog.Panel className="flex max-h-[100dvh] w-full max-w-2xl transform flex-col overflow-hidden rounded-t-2xl bg-white text-left align-middle shadow-xl transition-all sm:max-h-[90dvh] sm:rounded-2xl">
                                <div className="shrink-0 flex justify-between items-center border-b border-gray-100 p-4 sm:p-6">
                                    ...
                                    >
                                        ...
                                        {getContractStatusBadge(contract.estado)}
                                    </Dialog.Title>
                                   <button
                                        onClick={onClose}
                                ...
                                <div className="mb-6 flex gap-2 overflow-x-auto border-b border-gray-100 pb-1">
                                    <button
                                        ...
                                        className={`shrink-0 pb-3 px-3 text-sm font-medium transition-colors relative ${activeTab === 'general' ? 'text-indigo-600' : 'text-content-muted hover:text-gray-700'}`}
                                    >
                                        General
                                    ...
                                    <button
                                        ...
                                        className={`shrink-0 pb-3 px-3 text-sm font-medium transition-colors relative ${activeTab === 'financial' ? 'text-indigo-600' : 'text-content-muted hover:text-gray-700'}`}
                                    >
                                    ...
                                    <button
                                        ...
                                        className={`shrink-0 pb-3 px-3 text-sm font-medium transition-colors relative ${activeTab === 'cuotas' ? 'text-indigo-600' : 'text-content-muted hover:text-gray-700'}`}
                                    >
                                        Planes
                                    ...
                                    <button
                                        ...
                                        className={`shrink-0 pb-3 px-3 text-sm font-medium transition-colors relative ${activeTab === 'audit' ? 'text-indigo-600' : 'text-content-muted hover:text-gray-700'}`}
                                    >
                                        Auditoría
                                ...
                                {activeTab === 'general' ? (
                                        ...
                                        <div className="space-y-6">
                                                    ...
                                                    {contract.propietarios.map((p, idx) => (
                                                        <div key={p.id} className={`${idx > 0 ? 'pt-3 border-t border-gray-200' : ''}`}>
                                                            <div className="flex justify-between items-start">
                                                                ...
                                                                {p.esPrincipal && (
                                                                    <span className="text-xs font-bold text-status-accent bg-indigo-50 px-1.5 py-0.5 rounded">PRINCIPAL</span>
                                                                )}
                                                            </div>
                                            ...
                                            <div className="bg-indigo-50/50 p-4 rounded-xl border border-indigo-100">
                                                ...
                                                <div className="grid grid-cols-3 gap-4">
                                                    <div>
                                                        <p className="text-xs text-content-muted mb-1">Inicio</p>
                                                        <p className="text-sm font-bold text-gray-900">
                                                            {formatDate(contract.fechaInicio)}
                                                        </p>
                                                    </div>
                                                    <div>
                                                        <p className="text-xs text-content-muted mb-1">Fin</p>
                                                        <p className="text-sm font-bold text-gray-900">
                                                            {formatDate(contract.fechaFin)}
                                                        </p>
                                                    </div>
                                                    <div>
                                                        <p className="text-xs text-content-muted mb-1">Actualización</p>
                                                        <p className={`text-sm font-bold ${contract.requiereActualizacion ? 'text-indigo-600' : 'text-content-muted'}`}>
                                            ...
                                            {(contract.historialRenovaciones || []).length > 1 && (
                                                <div className="rounded-xl border border-violet-100 bg-violet-50/50 p-4">
                                                    <div className="mb-4 flex items-center gap-2">
                                                        <ArrowPathIcon className="h-5 w-5 text-violet-700" />
                                                        <div>
                                                            <h4 className="text-sm font-bold uppercase tracking-wide text-violet-950">Continuidad contractual</h4>
                                                            <p className="text-xs text-violet-800">Historial de contratos que se renuevan entre sí.</p>
                                                        </div>
                                                    </div>
                                                    <ol className="space-y-3 border-l-2 border-violet-200 pl-4">
                                                        {contract.historialRenovaciones?.map(entry => (
                                                            <li key={entry.id} className="relative rounded-lg border border-violet-100 bg-white p-3">
                                                                <span className={`absolute -left-[1.42rem] top-4 h-3 w-3 rounded-full border-2 border-white ${entry.id === contract.id ? 'bg-indigo-600 ring-2 ring-indigo-200' : 'bg-violet-400'}`} aria-hidden="true" />
                                                                <div className="flex flex-wrap items-center justify-between gap-2">
                                                                    <div>
                                                                        <p className="text-sm font-bold text-gray-900">Contrato #{entry.id}{entry.id === contract.id ? ' · Actual' : ''}</p>
                                                                        <p className="mt-0.5 text-xs text-content-muted">
                                                                            {entry.contratoAnteriorId ? `Renueva el contrato #${entry.contratoAnteriorId}` : 'Contrato de origen'} · {formatDate(entry.fechaInicio)} al {formatDate(entry.fechaRescision || entry.fechaFin)}
                                                                        </p>
                                                                    </div>
                                                                    <div className="text-right">
                                                                        {getContractStatusBadge(entry.estado)}
                                                                        <p className="mt-1 text-xs font-semibold text-gray-700">{formatCurrency(Number(entry.montoAlquiler), entry.moneda)}</p>
                                                                    </div>
                                                                </div>
                                                                {entry.inquilinoPrincipal && <p className="mt-2 text-xs text-gray-600">Inquilino principal: {entry.inquilinoPrincipal}</p>}
                                                            </li>
                                                        ))}
                                                    </ol>
                                                </div>
                                            ...
                                            {contract.estado === 'RESCINDIDO' && (
                                                <div className="rounded-xl border border-orange-200 bg-orange-50 p-4">
                                                    <div className="flex items-start gap-2">
                                                        <NoSymbolIcon className="mt-0.5 h-5 w-5 shrink-0 text-orange-700" />
                                                        <div>
                                                            <h4 className="text-sm font-bold text-orange-900">Rescisión registrada</h4>
                                                            <p className="mt-1 text-sm text-orange-900">Fecha efectiva: {formatDate(contract.fechaRescision || contract.fechaFin)}</p>
                                                            <p className="mt-1 text-sm text-orange-800">Motivo: {contract.motivoRescision || 'Sin detalle histórico'}</p>
                                                            {contract.rescindidoPor && <p className="mt-1 text-xs text-orange-700">Registrada por {contract.rescindidoPor.nombreCompleto}</p>}
                                                        </div>
                                                    </div>
                                                </div>
                                            )}
                                            ...
                                            <div className="bg-green-50/50 p-4 rounded-xl border border-green-100">
                                                <div className="flex items-center gap-2 mb-4">
                                                    <DocumentTextIcon className="w-5 h-5 text-status-success" />
                                                ...
                                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                                                    <div>
                                                        <p className="text-xs text-content-muted mb-1">Alquiler</p>
                                            ...
                                            {contract.actualizaciones && contract.actualizaciones.length > 0 && (
                                                <div className="bg-amber-50/50 p-4 rounded-xl border border-amber-100">
                                                    <div className="flex items-center gap-2 mb-4">
                                                        <ArrowPathIcon className="w-5 h-5 text-status-warning" />
                                            ...
                                            <div className="space-y-3">
                                                ...
                                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                                    {contract.rutaArchivoContrato && (
                                                        <div className="p-3 bg-indigo-50/60 border border-indigo-200 rounded-lg">
                                                            <div className="flex items-center justify-between gap-3">
                                                                <div className="flex min-w-0 items-center gap-2">
                                                                    <DocumentTextIcon className={`w-5 h-5 shrink-0 ${isWordDocument(contract.rutaArchivoContrato) ? 'text-blue-600' : 'text-indigo-500'}`} />
                                                                    <div className="min-w-0">
                                                                        <p className="text-sm font-bold text-gray-800 truncate">
                                                                            Contrato principal vigente{currentPrincipalDocument?.versionDocumento ? ` · Versión ${currentPrincipalDocument.versionDocumento}` : ''}
                                                                        </p>
                                                                        {currentPrincipalDocument && (
                                                                            <p className="text-xs text-gray-600">
                                                                                Incorporado el {formatDate(currentPrincipalDocument.fechaDocumento)}{currentPrincipalDocument.creadoPor?.nombreCompleto ? ` por ${currentPrincipalDocument.creadoPor.nombreCompleto}` : ''}
                                                                            </p>
                                                                        )}
                                                                    </div>
                                                                </div>
                                                            ...
                                                            {currentPrincipalDocument?.observacion && (
                                                                <p className="mt-2 text-xs text-gray-600 italic">{currentPrincipalDocument.observacion}</p>
                                                            )}
                                                        </div>
                                                    )}

                                                    {previousPrincipalDocuments.map((document) => (
                                                        <div key={document.id} className="p-3 bg-white border border-gray-200 rounded-lg">
                                                            <div className="flex items-center justify-between gap-3">
                                                                <div className="flex min-w-0 items-center gap-2">
                                                                    <DocumentTextIcon className={`w-5 h-5 shrink-0 ${isWordDocument(document.nombreArchivo || document.rutaArchivo) ? 'text-blue-600' : 'text-gray-600'}`} />
                                                                    <div className="min-w-0">
                                                                        <p className="text-sm font-medium text-gray-700 truncate">
                                                                            Contrato principal · Versión {document.versionDocumento || 'histórica'}
                                                                        </p>
                                                                        <p className="text-xs text-gray-500">
                                                                            {formatDate(document.fechaDocumento)}{document.creadoPor?.nombreCompleto ? ` · ${document.creadoPor.nombreCompleto}` : ''}
                                                                        </p>
                                                                    </div>
                                                                </div>
                                                                <button
                                                                    onClick={() => handleOpenDocument(document.rutaArchivo)}
                                                                    className="shrink-0 text-xs font-bold text-indigo-600 hover:text-indigo-800 transition-colors"
                                                                >
                                                                    {getDocumentActionLabel(document.nombreArchivo || document.rutaArchivo)}
                                                                </button>
                                                            </div>
                                                            {document.observacion && <p className="mt-2 text-xs text-gray-600 italic">{document.observacion}</p>}
                                                        </div>
                                                    ))}

                                                    {complementaryDocuments.map((document) => (
                                                        <div key={document.id} className="p-3 bg-white border border-gray-200 rounded-lg">
                                                            <div className="flex items-center justify-between gap-3">
                                                                <div className="flex min-w-0 items-center gap-2">
                                                                    <DocumentTextIcon className={`w-5 h-5 shrink-0 ${isWordDocument(document.nombreArchivo || document.rutaArchivo) ? 'text-blue-600' : 'text-gray-600'}`} />
                                                                    <div className="min-w-0">
                                                                        <p className="text-sm font-medium text-gray-700 truncate">
                                                                            {document.tipo === 'ADENDA' ? 'Adenda' : document.nombreArchivo || 'Adjunto'}
                                                                        </p>
                                                                        <p className="text-xs text-gray-500">
                                                                            {formatDate(document.fechaDocumento)}{document.creadoPor?.nombreCompleto ? ` · ${document.creadoPor.nombreCompleto}` : ''}
                                                                        </p>
                                    ...
                                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                                        {isLoadingHistory ? (
                                                ...
                                                {liquidaciones.map(liq => (
                                                    <div key={liq.id} className="border border-gray-100 rounded-xl p-4 hover:bg-gray-50/50 transition-colors">
                                                        <div className="flex justify-between items-start mb-3">
                                                            <div>
                                                                <p className="font-bold text-gray-900 capitalize">
                                                                    {formatMonthYear(liq.periodo)}
                                                                </p>
                                                                <p className="text-xs text-content-muted mt-0.5">Liquidación #{liq.id}</p>
                                                            </div>
                                                        ...
                                                        <div className="grid grid-cols-2 gap-4 text-sm mb-3">
                                                            <div>
                                                                <p className="text-xs text-content-muted">Neto a Cobrar</p>
                                                                <p className="font-bold text-gray-900">{formatMoney(Number(liq.netoACobrar), liq.moneda)}</p>
                                                            </div>
                                                            <div className="text-right">
                                                                <p className="text-xs text-content-muted">Cobrado</p>
                                                                <p className="font-bold text-status-success">
                                                                    {formatMoney(liq.pagos?.reduce((sum, p) => sum + Number(p.monto), 0) || 0, liq.moneda)}
                                                                </p>
                                                                {(liq.aplicacionesCredito?.length || 0) > 0 && <p className="mt-1 text-xs font-bold text-indigo-700">Saldo a favor aplicado: {formatMoney(liq.aplicacionesCredito?.reduce((sum, application) => sum + Number(application.monto), 0) || 0, liq.moneda)}</p>}
                                                            </div>
                                                        </div>
                                                                ...
                                                                {liq.pagos.map(pago => (
                                                                    <div key={pago.id} className="flex justify-between items-center text-xs">
                                                                        <div className="flex items-center gap-2">
                                                                            ...
                                                                            <span className="bg-white border border-gray-200 px-1.5 py-0.5 rounded text-xs font-medium text-content-muted">{pago.metodoPago}</span>
                                                                        </div>
                                            ...
                                            <div className="text-center py-12 bg-gray-50 rounded-xl border border-dashed border-gray-200">
                                                ...
                                                <p className="text-xs text-content-muted mt-1">Este contrato aún no tiene liquidaciones generadas.</p>
                                            </div>
                                        )}
                                    </div>
                                ) : activeTab === 'cuotas' ? (
                                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                                        <div className="flex justify-between items-center mb-4">
                                            <h4 className="text-sm font-bold text-gray-900 uppercase tracking-widest">Planes de cuotas</h4>
                                            {canEditLiquidations && (
                                                <button
                                        ...
                                        {isLoadingCuotas ? (
                                            ...
                                            <div className="space-y-4">
                                                {planesCuotas.map(plan => (
                                                    <div key={plan.id} className="bg-gray-50/50 rounded-xl border border-gray-100 p-4">
                                                        <div className="flex justify-between items-start mb-3">
                                                            <div>
                                                                ...
                                                                <div className="mt-2 flex flex-wrap items-center gap-2">
                                                                    {getPlanStatusBadge(plan.estado)}
                                                                    {plan.planOrigen && <span className="text-[11px] font-medium text-violet-700">Reprograma el plan #{plan.planOrigen.id}</span>}
                                                                    {(plan.planesReprogramados?.length || 0) > 0 && <span className="text-[11px] font-medium text-violet-700">Continuado por plan #{plan.planesReprogramados?.[0].id}</span>}
                                                                </div>
                                                            </div>
                                                            <div className="flex items-center gap-2">
                                                                <span className="text-sm font-black text-gray-900">{formatMoney(Number(plan.montoTotal), plan.moneda)}</span>
                                                                {canEditLiquidations && plan.estado === "VIGENTE" && (
                                                                    <div className="flex flex-wrap justify-end gap-1">
                                                                        <button onClick={() => setPlanLifecycleAction({ plan, action: "CANCELAR" })} className="rounded-md border border-orange-200 bg-orange-50 px-2 py-1 text-[10px] font-bold text-orange-700 hover:bg-orange-100">Cancelar</button>
                                                                        <button onClick={() => setPlanLifecycleAction({ plan, action: "CONDONAR" })} className="rounded-md border border-teal-200 bg-teal-50 px-2 py-1 text-[10px] font-bold text-teal-700 hover:bg-teal-100">Condonar</button>
                                                                        <button onClick={() => setPlanLifecycleAction({ plan, action: "REPROGRAMAR" })} className="rounded-md border border-violet-200 bg-violet-50 px-2 py-1 text-[10px] font-bold text-violet-700 hover:bg-violet-100">Reprogramar</button>
                                                                    </div>
                                                                )}
                                                            </div>
                                                        </div>
                                                        ...
                                                        {plan.fechaCierre && (
                                                            <p className="mb-3 rounded-lg border border-gray-100 bg-white px-3 py-2 text-xs text-gray-600">
                                                                Cerrado el {formatDate(plan.fechaCierre)}{plan.cerradoPor ? ` por ${plan.cerradoPor.nombreCompleto}` : ""}{plan.motivoCierre ? ` · ${plan.motivoCierre}` : ""}
                                                            </p>
                                                        )}
                                                                    ...
                                                                    <div className="flex items-center gap-3">
                                                                        ...
                                                                        <div>
                                                                            ...
                                                                            <p className="text-[11px] text-content-muted">
                                                                                Vence {formatMonthYear(cuota.fechaVencimiento)}
                                                                            </p>
                                                                        </div>
                                                                    </div>
                                                                    <div className="flex items-center gap-3">
                                                                        {cuota.liquidacion ? (
                                                                            <span className="text-xs font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded cursor-help" title={`En liquidación ${cuota.liquidacion.id}`}>
                                                                                En Liq. {formatMonthYear(cuota.liquidacion.periodo, true)}
                                                                            </span>
                                                                        ) : null}
                                                                        {getInstallmentStatusBadge(cuota.estado)}
                                                                    </div>
                                                                </div>
                                                            ))}
                                                        </div>
                                            ...
                                            <div className="text-center py-12 bg-gray-50 rounded-xl border border-dashed border-gray-200">
                                                ...
                                                <p className="text-xs text-content-muted mt-1">Carga préstamos o acuerdos para que se cobren en las liquidaciones.</p>
                                            </div>
                                        )}
                                    </div>
                                ) : activeTab === 'audit' ? (
                                    <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                                        ...
                                        <AuditTrail
                                            logs={contract.auditLogs}
                                            meta={contract.auditMeta}
                                            loading={isLoadingAudit}
                                            onPageChange={loadAuditPage}
                                            emptyText="Este contrato todavía no tiene eventos de auditoría."
                                        />
                                    </div>
                                ) : null}
                                ...
                                <div className="mt-8 flex flex-col gap-3 pt-6 border-t border-gray-100 sm:flex-row sm:items-center sm:justify-between">
                                    ...
                                    {onRenew && !hasRenewedContract && contract.estado !== 'PAPELERA' && (
                                        <button
                                            onClick={() => onRenew(contract)}
                                            className="flex min-h-11 items-center justify-center gap-2 rounded-xl border border-violet-200 bg-violet-50 px-4 text-sm font-medium text-violet-800 transition-colors hover:bg-violet-100"
                                        >
                                            <ArrowPathIcon className="w-5 h-5" />
                                            Renovar contrato
                                        </button>
                                    )}
                                    {onRescind && canEditContracts && (contract.estado === 'ACTIVO' || contract.estado === 'PROGRAMADO') && (
                                        <button
                                            onClick={() => onRescind(contract)}
                                            className="flex min-h-11 items-center justify-center gap-2 rounded-xl border border-orange-200 bg-orange-50 px-4 text-sm font-medium text-orange-800 transition-colors hover:bg-orange-100"
                                        >
                                    ...
                                    {canDeleteContracts ? (
                                        <button
                                            ...
                                            className="destructive-action flex min-h-11 items-center justify-center gap-2 rounded-xl px-4 text-sm font-medium transition-colors"
                                        >
        ...
        <PlanLifecycleModal
            isOpen={!!planLifecycleAction}
            plan={planLifecycleAction?.plan || null}
            action={planLifecycleAction?.action || null}
            onClose={() => setPlanLifecycleAction(null)}
            onConfirm={handlePlanLifecycle}
        />
        </>
    );
}
